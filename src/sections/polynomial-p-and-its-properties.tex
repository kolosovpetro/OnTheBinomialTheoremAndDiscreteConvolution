...
\begin{equation*}
    ...
    \coeffA{m}{0},
\end{equation*}
where $\coeffA{m}{r}$ are real coefficients by definition~\eqref{eq:def_coeff_a}.
...
\begin{table}[H]
    ...
    \end{tabular}
    \caption{Coefficients $\coeffA{m}{r}$.} \label{tab:table}
\end{table}
...
By definition~\eqref{eq:def_polynomial_p}, in extended form, the polynomial $\polynomialP{m}{b}{x}$ is
\begin{equation*}
    \begin{split}
        \polynomialP{m}{b}{x}
        ...
        &=\sum_{r=0}^{m} \sum_{j=0}^{r} (-1)^j x^{r-j} \binom{r}{j} \coeffA{m}{r} \sum_{k=0}^{b-1} k^{r+j} \\
        &=\sum_{r=0}^{m} \sum_{j=0}^{r} x^{r-j} \binom{r}{j} \coeffA{m}{r} \frac{(-1)^j}{r+j+1} \sum_{s=0}^{r+j}
        \binom{r+j+1}{s} \bernoulli{s} (b-1)^{r+j-s+1}
    \end{split}
\end{equation*}
However, by symmetry~\eqref{ppty_symmetry_of_polynomial_l} the $\polynomialP{m}{b}{x}$ may also be written in the form
\begin{equation*}
    \begin{split}
        \polynomialP{m}{b}{x}
        &=\sum_{k=1}^{b} \sum_{r=0}^{m} \coeffA{m}{r} k^r(x-k)^r
        =\sum_{k=1}^{b} \sum_{r=0}^{m} \coeffA{m}{r} k^r \sum_{t=0}^{r} (-1)^{r-t} x^t \binom{r}{t} k^{r-t} \\
        &=\sum_{t=0}^{m} x^t \underbrace{\sum_{k=1}^{b} \sum_{r=t}^{m} (-1)^{r-t} \binom{r}{t}
...
\end{equation*}
Polynomials $\polynomialX{3}{t}{j}, \; 0\leq t \leq 3$ are
\begin{equation*}
    \begin{split}
        \polynomialX{3}{0}{j}
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
It gives us an opportunity to review the $\polynomialP{m}{b}{x}$ from different prospective, for instance
\begin{equation}
